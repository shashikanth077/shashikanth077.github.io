...
Source of truth for `edit-pdf`'s design: a full-featured, browser-only PDF
editor. Written before implementation so the UI, data model, and phasing are
decided once instead of drifting tool-by-tool.

## 1. Editor layout and interaction model

The editor is a single full-width canvas app: a sticky icon toolbar above the
page stack, a bottom-sticky "Apply changes" CTA, and floating icons on the
right edge (page grid, attachments — Phase 3). Every placed element gets its
own floating mini-toolbar directly above it while selected.
...
| **Forms** ▾ | Two groups: *Add text & symbols* (free text, ✕, ✓, ●) and *Add new form fields* (Text, Text multiline, Drop-down list, Radio button, Checkbox, Signature box), plus *Change existing form fields* (Form Edit mode, Change tab order). |
...
| **Sign** ▾ | Shows previously-created signatures for one-click placement, plus **New Signature** → modal with **Type / Draw / Upload Image** tabs. Type tab: name field, 7-color swatch row, gallery of handwriting-style previews. |
...
| **Annotate** ▾ | *Show/Hide annotations* toggle. Strike out, Highlight, Underline (color swatch rows), and freehand Draw. |
...
`B` `I` · font size ▾ · font family ▾ · text color (palette icon) · link ·
...
`Create signature` header, three source tabs (Type / Draw / Upload Image), a
name textbox, 7 preset ink colors, and — only on the Type tab — a grid of
cursive/handwriting style previews rendered live from the typed name.
...
- No autosave to a server: nothing ever leaves the browser, so there is
  nothing to expire or discard server-side — nothing is ever uploaded in the
  first place.

## 2. What's achievable client-only
...
- **Achievable, full-featured client-side:** toolbar layout and grouping,
...
- **Not offered:** "Publish for others to fill & sign" (needs hosted storage
  + a server to receive submissions), camera signature capture, and
  Dropbox/Google Drive import (needs OAuth + a backend proxy). All are cut
  from scope rather than faked — a live-camera or cloud-import affordance
  that silently does nothing would be worse than not having it.
- **Editing existing embedded text ("best-effort real edit", per product
  decision):** see §3 — server-side content-stream rewriting against
  un-subsetted fonts has no safe client-side equivalent, so the
  patch-and-re-render strategy below stands in for it.

## 3. Editing existing text — best-effort strategy

A "real" content-stream text edit means rewriting the literal `Tj`/`TJ`
operators in a page's content stream, which requires full font-subset
introspection and a mature PDF graphics engine. `pdf-lib` does not expose
content-stream *mutation* of already-existing operators (only appending new
drawing calls), and blindly rewriting a subsetted font's glyph string
client-side risks emitting glyph codes the embedded font's subset doesn't
contain — i.e. corrupted output. Silently shipping that would violate the
"without issues" bar more than being upfront about the technique.
...
2. With the **Text** tool active, clicking inside an existing run's bounding
   ...
   that run — the same "click any existing text to start editing" gesture
   as adding new text.
...
Extend the existing `Annotation` union to the full element set. Each variant
keeps the existing convention: PDF-space coordinates (72pt/inch, bottom-left
origin), flattened by one `flattenAnnotations` pass at save time — nothing
new architecturally, just more variants:
...
type Annotation =
  ...
  | MarkupAnnotation // strikeout | underline over a dragged region
...
knows to draw the cover rectangle before drawing the new text — Phase 4 work.

### Component structure (`apps/pdf-tools/src/tools/edit-pdf/`)

```
tools/EditPdf.tsx                 – page orchestration, history, save
...
tools/edit-pdf/elements.tsx       – one renderer per annotation type
tools/edit-pdf/SignatureModal.tsx – Type/Draw/Upload signature creation
tools/edit-pdf/geometry.ts        – screen ⇄ PDF coordinate conversion
tools/edit-pdf/interactions.ts    – shared drag-move / corner-resize hooks
tools/edit-pdf/constants.ts       – shared color palettes, defaults
tools/edit-pdf/icons.tsx          – toolbar icon set
tools/edit-pdf/handwritingStyles.ts – signature Type-tab style gallery
tools/edit-pdf/PageChrome.tsx     – per-page toolbar (Phase 3: delete/zoom/rotate/insert)
tools/edit-pdf/textRuns.ts        – Phase 4: extracted-run cache + hit-testing
```
...
The icon-button toolbar maps directly onto tokens already in `tokens.css` /
`EditPdf.css` — no new color system needed:

- Toolbar background `var(--surface)`, sticky, `var(--shadow-sm)` — already
  the case.
- Active tool = `var(--accent)` fill — the platform's own emerald accent,
  consistent with the rest of the site per [feedback_browser_compat] and
  [feedback_design_quality].
- Dropdown panels: `.pdfed__dropdown` using `var(--surface)` +
...
`flattenAnnotations(pdfBytes, annotations)` → one pdf-lib pass → blob →
download. "Apply changes" is the single commit action — everything before it
is a live, undoable in-browser edit; the label was chosen because "Save &
Download" would misdescribe operations like whiteout or replace-text that
pre-composite content rather than just adding to it.

## 5. Phased delivery

Each phase ships independently typechecked, built (`npm run build`), and
verified in the running dev preview before the next starts.
